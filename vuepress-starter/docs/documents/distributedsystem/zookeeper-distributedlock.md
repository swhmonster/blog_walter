...
- ZooKeeper的每一个节点都是一个天然的顺序发号器
    ...
    例如，用一个发号的节点“/test/lock”为父亲节点，可以在这个父节点下面创建相同前缀的临时顺序子节点，假定相同的前缀为“/test/lock/seq-”。如果是第一个创建的子节点，那么生成的子节点为/test/lock/seq-0000000000，下一个节点则为/test/lock/seq-0000000001，依次类推。
    ![An image](../../zkdistributedlock.png)
