...
## Flink的优势
- 其他批处理引擎
    |流处理引擎|流模型|处理语义|容错|状态管理|内存管理|
    |---|---|---|---|---|---|
    |storm| 原生|at-least-once|ACK|无|JVM堆
    |kafka stream| 非原生（微批）|exactly-once|RDD checkpoint|部分算子|JVM堆
    |spark streaming|原生|exactly-once|分布式快照|算子支持状态|JVM堆外|
- Flink细节概念（个人理解）
    - 算子：一个一个计算单元，算子相连组成任务
    - 状态：计算产生的中间结果
### 同时支持高吞吐、低延迟、高性能
...
>checkpoint机制原理来源于“chandy-lanmport”算法论文
### 基于JVM实现独立的内存管理
...
## Flink基本架构
### 基本组件栈
![An image](../../flink-architecture.png)
### 基本架构图
![An image](../../flink-architecture2.png)
- 关注3大模块
    - Client客户端
    - JobManager管理节点
    - TaskManager执行节点
